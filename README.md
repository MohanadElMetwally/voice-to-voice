...
Message types are defined in `core/enums/message.py`.

### Audio encoding

- Input (client → server): base64‑encoded PCM16 mono. The STT session config (`schemas/stt.py`) declares `input_audio_format = "pcm16"` with server‑side VAD parameters.
- Output (server → client): base64‑encoded PCM `Raw24Khz16BitMonoPcm` from Azure Speech.

Client playback strategies:

- Wrap raw PCM in a WAV header and play via Web Audio API
- Or use an `AudioWorklet`/`ScriptProcessor` to enqueue raw PCM directly

## Interruption behavior

When `INTERRUPT_AGENT=true`:

- The STT client listens for `speech_started` events from the server VAD.
- On speech start, the server signals a cancellation token and sends `{ "type": "INTERRUPT" }` to the client.
- TTS streaming finishes early, and the pipeline is ready to handle the user’s new utterance.

This keeps conversations natural—talking over the assistant interrupts it.

## Troubleshooting

- STT/TTS endpoints must start with `wss://`; otherwise you’ll see configuration errors.
- If you receive `TTSConnectionError` or `TTSRuntimeError`, verify Speech keys/endpoint and that the voice name is valid.
- If STT never finalizes, adjust VAD settings: `AZURE_STT_THRESHOLD`, `AZURE_STT_PREFIX_PADDING_MS`, `AZURE_STT_SILENCE_DURATION_MS`.
- Ensure all required `.env` variables are set; missing keys will cause runtime failures.

## Project scripts and structure

```
pyproject.toml      # dependencies and build backend
src/voice_to_voice/
	main.py           # FastAPI app
	api/chat.py       # WebSocket endpoint
	services/chat/    # ChatService orchestrator
	infra/            # STT (realtime ws), TTS (streaming), Semantic Kernel agent
	core/config.py    # settings (env)
	schemas/, dto/    # pydantic models and message shapes
	utils/            # helpers (cancel token, transcript event, text cleanup)
```
